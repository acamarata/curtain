...
///          `public` because Settings.onIdle/Settings.onEnd (public computed
///          properties) return this type across the CurtainCore module boundary.
/// SPORT: MASTER-ACTIONS
public struct ActionSet {
    public var activateCurtain = false
    public var disconnect = false
    public var lock = false
    public var screenOff = false
    public var deactivateCurtain = false

    public init(
        activateCurtain: Bool = false, disconnect: Bool = false, lock: Bool = false,
        screenOff: Bool = false, deactivateCurtain: Bool = false
    ) {
        self.activateCurtain = activateCurtain
        self.disconnect = disconnect
        self.lock = lock
        self.screenOff = screenOff
        self.deactivateCurtain = deactivateCurtain
    }
}

/// Purpose: Test-injection seam for SessionCoordinator's `runner` property. The real
///          `ActionRunner` conforms; SessionCoordinatorTests inject a counting spy so
///          idempotency tests can assert invocation counts (e.g. "lock ran exactly
///          once on reconnect-while-active"), not just end state. Additive only —
///          `ActionRunner`'s behavior is unchanged, this just names its public surface
