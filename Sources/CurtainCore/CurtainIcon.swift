...
///          `public` because the thin `Curtain` executable (main.swift's
///          --render-icon build helper, MenuBarController, CoverContentView) calls
///          these across the CurtainCore module boundary.
/// SPORT: MASTER-ICON
public enum CurtainIcon {
    ...
    public static func exportIconset(to dir: String) {
    ...
    private static func draw(in rect: NSRect, monochrome: Bool) {
        ...
        drawPanel(
            ctx, x0: w * 0.10, x1: w * 0.46, top: panelTop, bottom: panelBottom,
            folds: 3, color: panelColor, fold: foldColor, mirrored: false)
        drawPanel(
            ctx, x0: w * 0.54, x1: w * 0.90, top: panelTop, bottom: panelBottom,
        ...
        rodColor.setFill()
        NSBezierPath(
            roundedRect: NSRect(x: w * 0.07, y: rod, width: w * 0.86, height: h * 0.055),
            xRadius: h * 0.03, yRadius: h * 0.03
        ).fill()
    }

    private static func drawPanel(
        _ ctx: CGContext, x0: CGFloat, x1: CGFloat, top: CGFloat, bottom: CGFloat,
        ...
        for i in 0..<segs {
            ...
            let mid = (sx + ex) / 2
            path.curve(
                to: NSPoint(x: ex, y: bottom + (top - bottom) * 0.06),
